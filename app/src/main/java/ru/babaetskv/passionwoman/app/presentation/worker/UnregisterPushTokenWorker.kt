...
class UnregisterPushTokenWorker(
    ...
    override suspend fun doWork(): Result =
        try {
            ...
            pushGateway.unregisterPushToken(token)
            Timber.i("Successfully unregistered push token")
            Result.success()
        } catch (e: Exception) {
            if (runAttemptCount + 1 >= MAX_ATTEMPT_COUNT) {
                Timber.i("Failed to unregister push token")
                Result.failure()
            } else {
                Timber.i("Failed to unregister push token. Retrying")
                Result.retry()
            }
        }
    ...
    class Factory(
        ...
        override fun create(appContext: Context, params: WorkerParameters): CoroutineWorker =
            UnregisterPushTokenWorker(appPreferences, pushGateway, appContext, params)
    }
